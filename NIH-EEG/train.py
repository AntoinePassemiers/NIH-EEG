...
def dropoutRate(inputs):
    ...
    for i in range(n_files):
        for j in range(n_features):
            dropouts[i, j] = float(np.count_nonzero(np.isnan(inputs[i][:, j]))) / float(T)
...
def main(validate = True): 
    ...
    for k in range(0, 3):
        ...
        NUM_PREICTAL_EXAMPLES_BY_MODEL = int(NUM_EXAMPLES_BY_MODEL * config.preictal_proportion)
        NUM_INTERICTAL_EXAMPLES_BY_MODEL = NUM_EXAMPLES_BY_MODEL - NUM_PREICTAL_EXAMPLES_BY_MODEL
        patient_classifiers = list()
        n_classifiers_for_this_patient = config.n_classifiers
        for j in range(n_classifiers_for_this_patient):
            filepaths  = p_filenames[k][j*NUM_PREICTAL_EXAMPLES_BY_MODEL:(j+1)*NUM_PREICTAL_EXAMPLES_BY_MODEL]
            filepaths += n_filenames[k][j*NUM_INTERICTAL_EXAMPLES_BY_MODEL:(j+1)*NUM_INTERICTAL_EXAMPLES_BY_MODEL]
            labels = [1] * NUM_PREICTAL_EXAMPLES_BY_MODEL + [0] * NUM_INTERICTAL_EXAMPLES_BY_MODEL
            ...
            if not os.path.isfile(os.path.join(model_path, "classifier_%i_%i" % (model_id, j))):
                inputs, outputs = list(), list()
                ...
                iohmm.pySave(os.path.join(model_path, "classifier_%i_%i" % (model_id, j)))
                
            else:
                iohmm.pyLoad(os.path.join(model_path, "classifier_%i_%i" % (model_id, j)))
                print("Classifier %i - %i loaded" % (model_id, j))
                
            patient_classifiers.append(iohmm)
        
        if validate == True:
            validation_results = open(os.path.join(DATA_PATH, "validation_%i.txt" % k), "w")
            inputs, outputs, filenames = randomValidationSet(k + 1, 500)
            predictions, ys = list(), list()
            for i in range(len(inputs)):
                label = outputs[i][-1]
                validation_results.write("Patient : %i / Label : %i\n" % (k, int(label)))
...
def test():
    ...
    for line in sample_submission:
        ...
        if patient_id == 9:
            prediction1 = classifiers[0][0].predictIO(input, binary_prediction = True)[0]
            prediction2 = classifiers[1][0].predictIO(input, binary_prediction = True)[0]
            prediction3 = classifiers[2][0].predictIO(input, binary_prediction = True)[0]
            prediction = 1.0/3.0 * prediction1 + 1.0/3.0 * prediction2 + 1.0/3.0 * prediction3
        else:
            prediction = classifiers[patient_id - 1][0].predictIO(input, binary_prediction = True)[0]
